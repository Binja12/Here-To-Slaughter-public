...
export abstract class Draw {
   ...
   * that many" — `-7` is Wily Red's "draw until you are holding seven cards",
   * `-5` is the redraw — a hand already there draws nothing (nobody ever draws
   * a negative number, so the sign is free to mean this). Stops early when the
 ...
 * the chosen seat with `executor: 'chosen'` ("that opponent may then draw one
 * card", Plundering Puma). Given a NUMBER it draws that many from the top, or
 * "until that many" when negative; given a SLOT NAME it draws the cards that
 * slot names out of wherever they lie in the deck — a card the player looked
 * at and chose (Bullseye: a choice over the deck's top three, then this).
 */
 ...
 * in it — Bullseye's "put the other two back on top in the order you choose":
 * the player picks which of the two is on top (a CardChoice over the deck's top
