...
describe('Bear Claw (hero-005)', () => {
  it('a pulled Hero card announces the condition WITH the chosen seat riding along', () => {
    ...
    const ctx = new AbilityContext('hero-005', 'p1')
  ...
  it('a pulled Item card announces nothing — one pull only', () => {
    ...
    seat(gs, 'p1')
    seat(gs, 'p2', ['theirs'])
    gs.registerCard(buildCard({ id: 'theirs', name: 'theirs', type: CardType.Item, image: '', description: '', set: 'base', cursed: false } as CardBase))
    const ctx = new AbilityContext('hero-005', 'p1')
    ctx.set(CTX_CHOSEN_PLAYER, ['p2'])
    const { em, emitted } = collect()
    const rm = new ReactionManager(gs, em)
