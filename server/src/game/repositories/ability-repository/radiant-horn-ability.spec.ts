...
describe('Radiant Horn (hero-027)', () => {
  it('fires on its own successful roll', () => {
    expect(RadiantHornAbility[0].trigger).toMatchObject({ on: GameEventType.RollSuccess, scope: 'SelfCard' })
  })

  it('offers only the Modifier cards in the discard pile, and the pick comes to hand', () => {
    const gs = makeGs()
    seat(gs, 'p1')
    gs.registerCard(new ModifierCard({ id: 'wanted', name: 'wanted', type: CardType.Modifier, image: '', description: '', set: 'base', values: [1] }))
    gs.registerCard(hero('other'))
    gs.getDiscardPile().add('wanted')
    gs.getDiscardPile().add('other')
    const ctx = new AbilityContext('hero-027', 'p1')

    const em = new GameEventEmitter()
    const rm = new ReactionManager(gs, em)
    const [choose, retrieve] = RadiantHornAbility[0].steps
