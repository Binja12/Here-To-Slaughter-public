...
describe('Call to the Fallen (magic-061)', () => {
  it('fires on its settled challenge frame', () => {
    expect(CallToTheFallenAbility[0].trigger).toMatchObject({ on: GameEventType.FrameResolved, scope: 'SelfCard' })
  })

  it('offers only the Hero cards in the discard pile, and the pick comes to hand', () => {
    const gs = makeGs()
    seat(gs, 'p1')
    gs.registerCard(hero('wanted'))
    gs.registerCard(item('other'))
    gs.getDiscardPile().add('wanted')
    gs.getDiscardPile().add('other')
    const ctx = new AbilityContext('magic-061', 'p1')

    const em = new GameEventEmitter()
    const rm = new ReactionManager(gs, em)
    const [choose, retrieve] = CallToTheFallenAbility[0].steps
