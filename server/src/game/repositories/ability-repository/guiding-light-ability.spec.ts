...
describe('Guiding Light (hero-025)', () => {
  it('fires on its own successful roll', () => {
    expect(GuidingLightAbility[0].trigger).toMatchObject({ on: GameEventType.RollSuccess, scope: 'SelfCard' })
  })
  ...
  it('offers only the Hero cards in the discard pile, and the pick comes to hand', () => {
    ...
    const ctx = new AbilityContext('hero-025', 'p1')

    const em = new GameEventEmitter()
    const rm = new ReactionManager(gs, em)
    const [choose, retrieve] = GuidingLightAbility[0].steps
