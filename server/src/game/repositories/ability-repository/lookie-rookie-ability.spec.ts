...
describe('Lookie Rookie (hero-011)', () => {
  it('fires on its own successful roll', () => {
    expect(LookieRookieAbility[0].trigger).toMatchObject({ on: GameEventType.RollSuccess, scope: 'SelfCard' })
  })

  it('offers only the Item cards in the discard pile, and the pick comes to hand', () => {
    const gs = makeGs()
    seat(gs, 'p1')
    gs.registerCard(item('wanted'))
    gs.registerCard(hero('other'))
    gs.getDiscardPile().add('wanted')
    gs.getDiscardPile().add('other')
    const ctx = new AbilityContext('hero-011', 'p1')

    const em = new GameEventEmitter()
    const rm = new ReactionManager(gs, em)
    const [choose, retrieve] = LookieRookieAbility[0].steps
